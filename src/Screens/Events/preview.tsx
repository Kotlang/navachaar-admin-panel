...
const EventPreview: React.FC<IEvent> = ({
	...
	return (
		<div className="container mx-auto p-4 bg-white rounded-lg shadow-md">
			...
			{mediaUrls && (
				<div className="mb-4">
					...
					{mediaUrls.map((media, index) => (
						<a key={index} href={media.url} className="text-blue-500 hover:underline">{media.url}</a>
					))}
				</div>
			)}
			...
			{webPreviews && (
				<div className="mb-4">
					<h2 className="text-lg font-semibold">Web Previews</h2>
					{webPreviews.map((preview, index) => (
						<a key={index} href={preview.url} className="text-blue-500 hover:underline">{preview.title}</a>
					))}
				</div>
			)}
		</div>
